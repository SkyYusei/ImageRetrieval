...
public:	
	...
	Feature<float>** kdtree_bbf( Feature<DataType> *feature,int neighbor_number, int max_nn );	/* Find the approximate K neighbors */
...
{	
	...
	feature_number = (int)(length / (feature_dimension * sizeof(DataType)));
...
template <typename DataType>
Feature<float>** Akmeans<DataType>::kdtree_bbf( Feature<DataType> *feature,int neighbor_number, int max_nn )
{
		...
		*explore_node = que.top();
		...
		while(explore_node->node)
		{
			...
			if(unexplore_node->node!=NULL)
			{
				...
				//que_push_with_MaxSize(que,unexplore_node,10000);
...
void Akmeans<DataType>::partition_features(Kdtreenode<float>* kdroot)
{
	...
	{
		kdroot->kd_right = new Kdtreenode<float>;
		kdtreenode_init(kdroot->kd_right,featuress+j+1,(n-j-1));
	}
}
