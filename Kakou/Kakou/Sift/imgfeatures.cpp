...
void draw_oxfd_feature( IplImage* img, struct feature* feat, CvScalar color )
{
	...
	// the original code lack of two paremeters behind, 
	// add them for higher version of opencv
...
void draw_my_feature (IplImage *img, struct feature *feat) {
	double x = feat->x;
	double y = feat->y;

	double len = 5;
	CvPoint lprt = cvPoint(x-len/2, y);
	CvPoint rprt = cvPoint(x+len/2, y);
	CvPoint uprt = cvPoint(x, y-len/2);
	CvPoint dprt = cvPoint(x, y+len/2);

	cvLine(img, lprt, rprt, cvScalar(255, 0, 0, 0), 1, 8, 0);
	cvLine(img, uprt, dprt, cvScalar(255, 0, 0, 0), 1, 8, 0);
}
