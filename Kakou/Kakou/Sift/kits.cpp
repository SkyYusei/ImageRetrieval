...
// here should add stdarg.h file, or link procedure will be failed
// commented by wlj
#include <stdarg.h>
...
/*
Combines two images by scacking one to the left of the other

@param img1 left image
@param img2 right image

@return Returns the image resulting from stacking \a img1 to the left if \a img2
*/
extern IplImage* queue_imgs( IplImage* img1, IplImage* img2 )
{
	IplImage* queued = cvCreateImage( cvSize( img1->width + img2->width,
										MAX(img1->height, img2->height)),
										IPL_DEPTH_8U, 3 );

	cvZero( queued );
	cvSetImageROI( queued, cvRect( 0, 0, img1->width, img1->height ) );
	cvAdd( img1, queued, queued, NULL );
	cvSetImageROI( queued, cvRect(img1->width, 0, img2->width, img2->height) );
	cvAdd( img2, queued, queued, NULL );
	cvResetImageROI( queued );
